...
def render_report(
    ...
    method_families = _count_by([summary.method_family for _, summary in papers])
    ...
    lines.extend(
        [
            ...
            f"- 方法类型：{_format_counts(method_families)}",
...
def _render_paper(index: int, paper: Paper, summary: PaperSummary) -> list[str]:
    ...
    return [
        ...
        f"- 方法类型：{summary.method_family}",
        f"- 研究类型：{summary.study_type}",
        f"- 任务/领域：{summary.task_or_domain}",
        f"- 数据集/基准：{summary.dataset_or_benchmark}",
        f"- 模型/系统：{summary.model_or_system}",
        f"- 评价指标：{summary.evaluation_metrics}",
